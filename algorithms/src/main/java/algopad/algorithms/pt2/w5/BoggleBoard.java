...
public class BoggleBoard {
    ...
    public BoggleBoard() {
        ...
        for (int i = 0; i < M; i++) {
            for (int j = 0; j < N; j++) {
                final String letters = BOGGLE_1992[N*i+j];
                final int r = StdRandom.uniform(letters.length());
    ...
    public BoggleBoard(final String filename) {
        ...
        for (int i = 0; i < M; i++) {
            for (int j = 0; j < N; j++) {
                final String letter = in.readString().toUpperCase();
                if ("QU".equals(letter)) {
                    board[i][j] = 'Q';
    ...
    public BoggleBoard(final int M, final int N) {
        ...
        this.N = N;
        if (M <= 0) {
            throw new IllegalArgumentException("number of rows must be a " +
                                               "positive integer");
        }
        if (N <= 0) {
            throw new IllegalArgumentException("number of columns must be a " +
                                               "positive integer");
        }
        board = new char[M][N];
        for (int i = 0; i < M; i++) {
            for (int j = 0; j < N; j++) {
                final int r = StdRandom.discrete(FREQUENCIES);
    ...
    public static void main(final String[] args) {
        ...
        final BoggleBoard board1 = new BoggleBoard();
        ...
        final BoggleBoard board2 = new BoggleBoard(4, 4);
        ...
        final BoggleBoard board3 = new BoggleBoard(a);
        StdOut.println(board3);
        StdOut.println();

        // initialize a 4-by-4 board from a file
        final String filename = "board-quinquevalencies.txt";
        StdOut.println("4-by-4 board from file " + filename + ':');
        final BoggleBoard board4 = new BoggleBoard(filename);
